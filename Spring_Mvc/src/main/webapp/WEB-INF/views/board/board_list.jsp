 ...
 <c:if test="${!empty blist}"> <%-- c:if는 jstl 조건문이다. --%>
  <c:forEach var="b" items="${blist}"> <%-- c:forEach는 JSTL 반복문이다. --%>
   <tr>
    ...
    ${b.title}</a>
    <%-- board_cont?bno=번호&page=쪽번호&state=cont get방식으로 3개의 피라미터값이 전달된다. --%>
    <c:if test="${b.replycnt != 0}"> <%-- 댓글이 있는 경우 실행 --%>
    &nbsp;&nbsp;<strong style="color:red;font-size:16px; padding:2px; background-color:gold; border-radius:3px;
    box-shadow:3px 3px 3px gray;">[댓글 개수: ${b.replycnt } 개]</strong>
    </c:if>
    </th>
    <th>${b.writer}</th>
    <th>${b.viewcnt}</th>
